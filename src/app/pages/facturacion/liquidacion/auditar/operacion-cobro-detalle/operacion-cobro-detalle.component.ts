...
export class OperacionCobroDetalleComponent implements OnInit {
  ...
  rendirCaja() {
    ...
    for (i = 0; i < this.selecteditems.length; i++) {
      this.total_facturado_impresion =
        this.total_facturado_impresion +
      ...
      if (complejidad == "1") {
        ...
        this.total_facturado_consulta =
          this.total_facturado_consulta +
          Number(this.selecteditems[i].valor_facturado);
      }
      if (complejidad == "2") {
        console.log(this.selecteditems[i].valor_facturado);
        this.total_facturado_practica =
          this.total_facturado_practica +
          Number(this.selecteditems[i].valor_facturado);
      }
    }
    console.log(this.total_facturado_practica);
    this._total_facturado_practica = this.cp.transform(
      this.total_facturado_practica,
      "",
      "symbol-narrow",
      "1.2-2"
    ...
    this._total_facturado_practica_porcentaje = this.cp.transform(
      this.total_facturado_practica,
      "",
      "symbol-narrow",
      "1.2-2"
    );
    this._total_facturado_consulta = this.cp.transform(
      this.total_facturado_consulta,
      "",
      "symbol-narrow",
      "1.2-2"
    );
    this._total_facturado_impresion = this.cp.transform(
      this.total_facturado_impresion,
      "",
      "symbol-narrow",
      "1.2-2"
          ...
          (resp) => {
            if (resp[0]) {
              ...
              for (let i = 0; i < resp.length; i++) {
                let myStr: string = this.padLeft(
                  resp[i]["operacion_cobro_numero_bono"],
                  "0",
                  10
                );
                ...
                if (resp[i]["complejidad"] == "2") {
                  resp[i]["distribucion"] = Number(
                    resp[i]["distribucion"] * 0.5
                  );
                }
      ...
      for (i = 0; i < this.result_distribucion.length; i++) {
        if (this.result_distribucion[i]["obra_social_id"] === 1) {
          ...
          if (
            ...
            doc.text(
              this.result_distribucion[i]["categorizacion"],
              140,
              y_honorarios,
              null,
              null,
