...
export class OperacionCobroAuditarComponent implements OnInit {
  ...
  editarOperacionCobroDistribucion() {
    ...
    ref.onClose.subscribe(
      (PopupCobroDistribucionEditarComponent: OperacionCobroDetalle) => {
        // this.loadRegistro();
        if (PopupOperacionCobroEditarComponent) {
          console.log(PopupOperacionCobroEditarComponent);
          //  this.popItemOperacionCobro = PopupOperacionCobroEditarComponent;
  ...
  afectarRegistros() {
    ...
    swal({
      ...
      text:
        "Va a auditar una liquidacion con fecha desde " +
        td +
        " y fecha hasta " +
        th +
        " para la obra social " +
          ...
          (resp) => {
            ...
            if (resp[0]) {
              for (let i = 0; i < resp.length; i++) {
                ...
                if (
                  resp[i]["forma_pago"] !== "EFECTIVO" &&
                  resp[i]["complejidad"] !== "3"
                ) {
      ...
      this.miServicio.auditarpractica(this.selecteditems).subscribe(
        (resp) => {
          ...
          if (resp != 0) {
            this.throwAlert(
              "success",
              "Se auditaron con éxito " + resp + " registros",
              "",
              ""
            );
            this.loadRegistro();
