...
export class PopupOperacionCobroPresentacionComponent implements OnInit {
  ...
  verRegistro() {
    ...
    ref.onClose.subscribe(
      (PopupOperacionCobroDistribucionComponent: OperacionCobroDetalle) => {
  ...
  editarPresentacion() {
    ...
    ref.onClose.subscribe(
      (
        PopupOperacionCobroPresentacionEditarRegistroComponent: OperacionCobroDetalle
      ) => {
        this.loadRegistro();
  ...
  loadRegistroByIdLiquidacion() {
    ...
    try {
      this.miServicio
        .getOperacionCobroRegistrosByLiquidacionNumero(this.config.data.id)
        .subscribe(
          (resp) => {
            if (resp[0]) {
              let i: number = 0;
